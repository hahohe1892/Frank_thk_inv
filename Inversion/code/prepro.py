...
import numpy as np
from topg_inv import internal_buffer, filter_tf
...
def initialize(params, state):
    ...
    # set aside starting conditions
    state.usurf_start = state.usurf.numpy()
    state.mask_start = state.mask.numpy()
    ...
    state.mask = tf.cast(state.mask, tf.float32)
    ...
    state.velsurf_magobs = state.vel_itslive * params.vel_mult
...
def _clean_masks(params, state):
    ...
    dt = np.maximum(state.mask.numpy() - state.TermMask.numpy(), 0)
    ...
    labeled_raster[state.mask.numpy() == 0] = np.nan
    ...
    state.mask = tf.Variable(out_mask)
