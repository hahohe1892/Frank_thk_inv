...
def initialize(params, state):
    ...
    if (params.mask_buffer > 0) & (hasattr(state, 'mask_count')):
        ...
        state.no_theta_area = state.mask - state.icemask
...
def update(params, state):
    ...
    if state.it>=0:
        ...
        if (state.t>0 and state.t%params.t_smb_update==0) or (state.it>0 and state.it%params.it_smb_update==0):
            print('updating surface mass balance...')
            _update_mass_balance(params, state)
